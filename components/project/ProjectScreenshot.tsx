...
export function ProjectScreenshot({ src, alt }: { src: string; alt: string }) {
  ...
  return (
    // 세로가 긴 것과 가로가 긴 것이 섞여 있어 비율을 이쪽에서 16/10으로 고정한다.
    // 그래야 세 프로젝트의 이미지 높이가 같고, 로드 전후로 레이아웃이 밀리지 않는다.
    ...
    <div className="relative aspect-[16/10] w-full overflow-hidden rounded-md border border-border">
      <Image
        ...
        alt={alt}
        fill
        // 본문 폭이 곧 이미지 폭이다. 본문은 넓은 화면에서 폭이 고정되므로 px로 적는다.
        sizes="(min-width: 1024px) 672px, (min-width: 768px) 720px, calc(100vw - 2rem)"
